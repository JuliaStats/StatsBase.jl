...
@testset "StatsBase.Weights" begin
    ...
    @testset "$f" for f in weight_funcs
        ...
        @test isa(f([1.0, 2.0, 3.0]), AbstractWeights{Float64})
    ...
    @testset "$f, setindex!" for f in weight_funcs
        ...
        @test wv[1] === 4.0                  # value correctly converted and set
        @test sum(wv) === 9.0                # sum updated
        @test wv == [4.0, 2.0, 3.0]    # Test state of all values
        ...
        @test wv[2] === 5.0
        @test sum(wv) === 11.0
        @test wv == [3.0, 5.0, 3.0]   # Test state of all values
    ...
    @testset "Mean $f" for f in weight_funcs
        ...
        for wt in ([1.0, 1.0, 1.0], [1.0, 0.2, 0.0], [0.2, 0.0, 1.0])
            @test mean(a, f(wt), dims=1) ≈
                  sum(a .* reshape(wt, length(wt), 1, 1), dims=1)/sum(wt)
            @test mean(a, f(wt), dims=2) ≈
                  sum(a .* reshape(wt, 1, length(wt), 1), dims=2)/sum(wt)
            @test mean(a, f(wt), dims=3) ≈
                  sum(a .* reshape(wt, 1, 1, length(wt)), dims=3)/sum(wt)
    ...
    @testset "Quantile fweights" begin
        ...
        for i in 1:length(data)
            @test quantile(data[i], fweights(fill!(similar(wt[i]), 1)), p) ≈
                  quantile(data[i], p)
        end

        # Issue #313
        @test quantile([1, 2, 3, 4, 5], fweights([0, 1, 2, 1, 0]), p) ≈
              quantile([2, 3, 3, 4], p)
        @test quantile([1, 2], fweights([1, 1]), 0.25) ≈ 1.25
        @test quantile([1, 2], fweights([2, 2]), 0.25) ≈ 1.0

        # test non integer frequency weights
        quantile([1, 2], fweights([1.0, 2.0]), 0.25) ==
        quantile([1, 2], fweights([1, 2]), 0.25)
    ...
    @testset "Quantile aweights, pweights and weights" for f in
                                                           (aweights, pweights, weights)
        data = ([7, 1, 2, 4, 10],
                [7, 1, 2, 4, 10],
                [7, 1, 2, 4, 10, 15],
                [1, 2, 4, 7, 10, 15],
        ...
        wt = ([1, 1/3, 1/3, 1/3, 1],
              ...
              [0.1, 0.2, 0.3, 0.2, 0.1],
              [1, 1, 1, 1, 1])
        quantile_answers = ([1.0, 4.0, 6.0, 8.0, 10.0],
    ...
    @testset "Exponential Weights" begin
        ...
        @testset "Usage" begin
            ...
            @testset "indexin" begin
                ...
                @test eweights(t1:Hour(2):tx, t1:Hour(1):tn, λ; scale=true) ≈
                      Weights(v[[1, 3, 5, 7]])
            end
        end
    ...
    @testset "custom weight types" begin
        ...
        @test isnan(mean([1, 2, 3], MyWeights([NaN, 4, 10])))
