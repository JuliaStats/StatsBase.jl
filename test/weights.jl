...
@testset "StatsBase.Weights" begin
    ...
    @testset "Quantile aweights, pweights and weights" for f in (aweights, pweights, weights)
        ...
        quantile_answers = (
            ...
            [1.0, 4.75, 7.5, 10.4166667, 15.0],
            [0.0, 2.617801, 5.2356021, 7.8534031, 20.0],
        ...
        Random.seed!(10)
        for i in 1:length(data)
            @test quantile(data[i], f(wt[i]), p) ≈ quantile_answers[i] atol = 1.0e-5
            for j in 1:10
                # order of p does not matter
                reorder = sortperm(rand(length(p)))
                @test quantile(data[i], f(wt[i]), p[reorder]) ≈ quantile_answers[i][reorder] atol = 1.0e-5
            end
            for j in 1:10
                # order of w does not matter
                reorder = sortperm(rand(length(data[i])))
                @test quantile(data[i][reorder], f(wt[i][reorder]), p) ≈ quantile_answers[i] atol = 1.0e-5
            end
        end
        ...
        for v in (1, 2, 345)
            for i in 1:length(data)
                ...
                @test quantile(data[i], w, p) ≈ quantile(data[i], p) atol = 1.0e-5
                for j in 1:10
                    prandom = rand(4)
                    @test quantile(data[i], w, prandom) ≈ quantile(data[i], prandom) atol = 1.0e-5
                end
            end
        end
        # test zeros are removed
        for i in 1:length(data)
            @test quantile(vcat(1.0, data[i]), f(vcat(0.0, wt[i])), p) ≈ quantile_answers[i] atol = 1.0e-5
        end
        # Syntax
        v = [7, 1, 2, 4, 10]
        w = [1, 1 / 3, 1 / 3, 1 / 3, 1]
        answer = 6.0
        @test quantile(data[1], f(w), 0.5) ≈ answer atol = 1.0e-5
    ...
    @testset "Median $f" for f in weight_funcs
        ...
        data = [4, 3, 2, 1]
        wt = [1, 2, 3, 4]
        @test median(data, f(wt)) ≈ quantile(data, f(wt), 0.5) atol = 1.0e-5
