...
@testset "StatsBase.Moments" begin
    ...
    @testset "Uncorrected with $f (values of type $(eltype(x)), weights of type $(eltype(w)))" for f in
                                                                                                   weight_funcs,
                                                                                                   x in
                                                                                                   (x,
                                                                                                    xf0),
    ...
    @testset "Corrected with $(weight_funcs[i]) (values of type $(eltype(x)), weights of type $(eltype(w)))" for i in
                                                                                                                 eachindex(weight_funcs),
                                                                                                                 x in
                                                                                                                 (x,
                                                                                                                  xf0),
                                                                                                                 w in
    ...
    @testset "Uncorrected with $f (values of type $(eltype(x)), 1st weights of type $(eltype(w1)), 2nd weights of type $(eltype(w2)))" for f in
                                                                                                                                           weight_funcs,
                                                                                                                                           x in
                                                                                                                                           (x,
                                                                                                                                            xf0),
        ...
        @testset "Variance" begin
            ...
            @test @inferred(var(x, wv1, 1; mean=m1, corrected=false))::Matrix{T1} ≈
                  expected_var1
            @test @inferred(var(x, wv2, 2; mean=m2, corrected=false))::Matrix{T2} ≈
                  expected_var2
            @test @inferred(varm(x, wv1, m1, 1; corrected=false))::Matrix{T1} ≈
                  expected_var1
            @test @inferred(varm(x, wv2, m2, 2; corrected=false))::Matrix{T2} ≈
                  expected_var2
        end
        ...
        @testset "Standard Deviation" begin
            ...
            @test @inferred(std(x, wv1, 1; mean=m1, corrected=false))::Matrix{T1} ≈
                  expected_std1
            @test @inferred(std(x, wv2, 2; mean=m2, corrected=false))::Matrix{T2} ≈
                  expected_std2
            @test @inferred(stdm(x, wv1, m1, 1; corrected=false))::Matrix{T1} ≈
                  expected_std1
            @test @inferred(stdm(x, wv2, m2, 2; corrected=false))::Matrix{T2} ≈
                  expected_std2
        end

        @testset "Mean and Variance" begin
            for d in 1:2
                (m, v) = @inferred(mean_and_var(x, d; corrected=false))::Tuple{Matrix{TX},
                                                                               Matrix{TX}}
                @test m == mean(x, dims=d)
                @test v == var(x, dims=d, corrected=false)
            end

            (m, v) = @inferred(mean_and_var(x, wv1, 1; corrected=false))::Tuple{Matrix{T1},
                                                                                Matrix{T1}}
            @test m == mean(x, wv1, dims=1)
            @test v == var(x, wv1, 1; corrected=false)

            (m, v) = @inferred(mean_and_var(x, wv2, 2; corrected=false))::Tuple{Matrix{T2},
                                                                                Matrix{T2}}
        ...
        @testset "Mean and Standard Deviation" begin
            for d in 1:2
                (m, s) = @inferred(mean_and_std(x, d; corrected=false))::Tuple{Matrix{TX},
                                                                               Matrix{TX}}
                @test m == mean(x, dims=d)
                @test s == std(x, dims=d; corrected=false)
            end

            (m, s) = @inferred(mean_and_std(x, wv1, 1; corrected=false))::Tuple{Matrix{T1},
                                                                                Matrix{T1}}
            @test m == mean(x, wv1, dims=1)
            @test s == std(x, wv1, 1; corrected=false)

            (m, s) = @inferred(mean_and_std(x, wv2, 2; corrected=false))::Tuple{Matrix{T2},
                                                                                Matrix{T2}}
            @test m == mean(x, wv2, dims=2)
    ...
    @testset "Corrected with $f (values of type $(eltype(x)), weights of type $(eltype(w1)))" for f in
                                                                                                  weight_funcs,
                                                                                                  x in
                                                                                                  (Float32.(x),
                                                                                                   Float64.(x)),
        ...
        if !isa(wv1, Weights)
            expected_var1 = sum(abs2.(x .- m1) .* w1, dims=1) .*
                            StatsBase.varcorrection(wv1, true)
            expected_var2 = sum(abs2.(x .- m2) .* w2', dims=2) .*
                            StatsBase.varcorrection(wv2, true)
            expected_std1 = sqrt.(expected_var1)
            expected_std2 = sqrt.(expected_var2)
        end
        ...
        @testset "Variance" begin
            if isa(wv1, Weights)
                ...
                @test @inferred(var(x, wv1, 1; mean=m1, corrected=true))::Matrix{T1} ≈
                      expected_var1
                @test @inferred(var(x, wv2, 2; mean=m2, corrected=true))::Matrix{T2} ≈
                      expected_var2
                @test @inferred(varm(x, wv1, m1, 1; corrected=true))::Matrix{T1} ≈
                      expected_var1
                @test @inferred(varm(x, wv2, m2, 2; corrected=true))::Matrix{T2} ≈
                      expected_var2
            end
        end
        ...
        @testset "Standard Deviation" begin
            if isa(wv1, Weights)
                ...
                @test @inferred(std(x, wv1, 1; mean=m1, corrected=true))::Matrix{T1} ≈
                      expected_std1
                @test @inferred(std(x, wv2, 2; mean=m2, corrected=true))::Matrix{T2} ≈
                      expected_std2
                @test @inferred(stdm(x, wv1, m1, 1; corrected=true))::Matrix{T1} ≈
                      expected_std1
                @test @inferred(stdm(x, wv2, m2, 2; corrected=true))::Matrix{T2} ≈
                      expected_std2
            end
        end
        ...
        @testset "Mean and Variance" begin
            for d in 1:2
                (m, v) = @inferred(mean_and_var(x, d; corrected=true))::Tuple{Matrix{TX},
                                                                              Matrix{TX}}
                @test m == mean(x, dims=d)
            ...
            if isa(wv1, Weights)
                ...
                (m, v) = @inferred(mean_and_var(x, wv1, 1; corrected=true))::Tuple{Matrix{T1},
                                                                                   Matrix{T1}}
                @test m == mean(x, wv1, dims=1)
                @test v == var(x, wv1, 1; corrected=true)

                (m, v) = @inferred(mean_and_var(x, wv2, 2; corrected=true))::Tuple{Matrix{T2},
                                                                                   Matrix{T2}}
                @test m == mean(x, wv2, dims=2)
        ...
        @testset "Mean and Standard Deviation" begin
            for d in 1:2
                (m, s) = @inferred(mean_and_std(x, d; corrected=true))::Tuple{Matrix{TX},
                                                                              Matrix{TX}}
                @test m == mean(x, dims=d)
            ...
            if isa(wv1, Weights)
                ...
                (m, s) = @inferred(mean_and_std(x, wv1, 1; corrected=true))::Tuple{Matrix{T1},
                                                                                   Matrix{T1}}
                @test m == mean(x, wv1, dims=1)
                @test s == std(x, wv1, 1; corrected=true)

                (m, s) = @inferred(mean_and_std(x, wv2, 2; corrected=true))::Tuple{Matrix{T2},
                                                                                   Matrix{T2}}
                @test m == mean(x, wv2, dims=2)
    ...
    @testset "Skewness and Kurtosis with $f" for f in weight_funcs
        for T in (Int, Float32, Float64)
            ...
            for n in (length(x) - 1, length(x) + 1)
                @test_throws DimensionMismatch("Inconsistent array lengths.") kurtosis(v,
                                                                                       f(ones(T,
                                                                                              n)))
                @test_throws DimensionMismatch("Inconsistent array lengths.") skewness(v,
                                                                                       f(ones(T,
                                                                                              n)))
            end
        end
        ...
        for T in (Int, Float32, Float64)
            ...
            for k in 1:6
                ...
                elseif k == 5
                    ...
                    @assert k == 6
                    @test cumk ≈
                          moment(x, 6) - 15*moment(x, 4)*moment(x, 2) - 10*moment(x, 3)^2 +
                          30*moment(x, 2)^3
                end
            end
            ...
            for TM in (Int, Float32, Float64)
                ...
                for k in 1:6
                    ...
                    elseif k == 5
                        ...
                        @assert k == 6
                        @test cumk ≈
                              moment(x, 6, m) - 15*moment(x, 4, m)*moment(x, 2, m) -
                              10*moment(x, 3, m)^2 + 30*moment(x, 2, m)^3
                    end
                end
            ...
            for n in (length(x) - 1, length(x) + 1), krange in (1, 1:3)
                @test_throws DimensionMismatch("Inconsistent array lengths.") cumulant(x,
                                                                                       krange,
                                                                                       f(ones(n)))
                @test_throws DimensionMismatch("Inconsistent array lengths.") cumulant(x,
                                                                                       krange,
