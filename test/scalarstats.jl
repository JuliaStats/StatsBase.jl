...
@testset "quantilerank and percentilerank" begin
    @testset "value as number and array" begin
        @testset ":inc and :exc" begin
            ...
            @test percentilerank(v2, 7, method=:exc) ==
                  100 * quantilerank(v2, 7, method=:exc)
        end
        ...
        @testset ":strict, :weak and :tied" begin
            ...
            for (method, res1, res2) in [(:tied, 0.4, [0.4, 0.85]),
                                         (:strict, 0.3, [0.3, 0.8]),
                                         (:weak, 0.5, [0.5, 0.9])]
...
@test mad(skipmissing([missing; 1:5; missing]); center=3, normalize=true) ≈
      1.4826022185056018
...
logpattern = (:warn,
              "support for empty collections will be removed since they do not represent proper probability distributions")
