...
@test isapprox(genmean([1, 1, 2, 3], -1.0e-8), (6.0)^(1 / 4), atol = 1.0e-8)
# Test numerical stability for large `p` (genmean should be close to max).
@test isapprox(genmean([0.98, 1.02], 1.0e4), 1.02, atol = 1.0e-4)
...
@test sort(modes(d1, weights([0.9, 0.1, 0.1, 0.1, 0.9, 0.1, 0.1, 0.1]))) == [1, 4]
...
z1 = [4.0 6.0 8.0 10.0; 5.0 6.0 -1.0 0.0; 1.5 3.0 0.0 -1.5]
z2 = [8.0 2.0 3.0 1.0; 24.0 10.0 -1.0 -1.0; 20.0 12.0 1.0 -2.0]
...
@test variation([1:5;]) ≈ 0.52704627669473
@test variation([1:5;]; corrected = false) ≈ 0.471404520791032
@test variation(skipmissing([missing; 1:5; missing])) ≈ 0.52704627669473
...
@test @inferred(sem([1:5;], ProbabilityWeights([1:5;]))) ≈ 0.6166 rtol = 0.001
μ = mean(1:5, ProbabilityWeights([1:5;]))
@test @inferred(sem([1:5;], ProbabilityWeights([1:5;]); mean = μ)) ≈ 0.6166 rtol = 0.001
@test @inferred(sem([10; 1:5;], ProbabilityWeights([0; 1:5;]); mean = μ)) ≈ 0.6166 rtol = 0.001
...
@inferred sem([1:5.0f0;]; mean = μ) ≈ sem(x)
@inferred sem([1:5.0f0;], ProbabilityWeights([1:5;]); mean = μ)
@inferred sem([1:5.0f0;], FrequencyWeights([1:5;]); mean = μ)
...
@test @inferred(isnan(sem(Int[]; mean = 0.0f0)))
@test @inferred(isnan(sem(Int[], FrequencyWeights(Int[]); mean = 0.0f0)))
@test @inferred(isnan(sem(Int[], ProbabilityWeights(Int[]); mean = 0.0f0)))
...
@test @inferred(genvar(nutrient)) ≈ 2.8310418e19 rtol = 1.0e-6
@test @inferred(totalvar(nutrient)) ≈ 2.83266877e6 rtol = 1.0e-6
...
@test @inferred(crossentropy([1 // 5, 3 // 10, 1 // 2], [0.3f0, 0.4f0, 0.3f0], 2.0f0)) isa Float32
...
@test @inferred(kldivergence([1 // 5, 3 // 10, 1 // 2], [0.3f0, 0.4f0, 0.3f0], 2.0f0)) isa Float32
@test iszero(@inferred(kldivergence([0, 1], [0.0f0, 1.0f0])))
