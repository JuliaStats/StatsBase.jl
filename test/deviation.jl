...
@testset "counting (arrays with element types $T1 and $T2)" for T1 in
                                                                (Int, Float32, Float64),
                                                                T2 in
                                                                (Int, Float32, Float64)
    ...
    for (a, b) in
        ((a, empty_b), (empty_a, b), (a_offset, empty_b_offset), (empty_a_offset, b_offset))
        @test_throws err counteq(a, b)
        @test_throws err countne(a, b)
    end
end

@testset "deviation (arrays with element types $T1 and $T2)" for T1 in (Float32, Float64),
                                                                 T2 in (Float32, Float64)
    ...
    for (a, b) in ((a, b), (a_offset, b_offset))
        ...
        @test @inferred(rmsd(a, b; normalize=true))::T ≈
              rmsd(a, b) / (maximum(a) - minimum(a))
