...
@testset "StatsBase.Covariance" begin
    ...
    function test_isapprox_preserves_symherm_structure(f::F, x::AbstractMatrix,
                                                       y::AbstractMatrix, args...) where {F}
        for wrapper in
            (identity, x -> Symmetric(x, :U), x -> Symmetric(x, :L), x -> Hermitian(x, :U),
             x -> Hermitian(x, :L))
            A = wrapper(copy(x))
    ...
    @testset "$f" for f in weight_funcs
        ...
        @testset "Uncorrected" begin
            @testset "Weighted Covariance" begin
                ...
                @test StatsBase.covm(X, mean(X, wv1, dims=1), wv1, 1; corrected=false) ≈
                      S1w ./ sum(wv1)
                @test StatsBase.covm(X, mean(X, wv2, dims=2), wv2, 2; corrected=false) ≈
                      S2w ./ sum(wv2)

                @test StatsBase.covm(X, zeros(1, 8), wv1, 1; corrected=false) ≈
                      Sz1w ./ sum(wv1)
                @test StatsBase.covm(X, zeros(3), wv2, 2; corrected=false) ≈
                      Sz2w ./ sum(wv2)
            end
            ...
            @testset "Conversions" begin
                ...
                @testset "cov2cor" begin
                    test_isapprox_preserves_symherm_structure(cov2cor, cov(X, dims=1),
                                                              cor(X, dims=1),
                                                              std(X, dims=1))
                    test_isapprox_preserves_symherm_structure(cov2cor, cov(X, dims=2),
                                                              cor(X, dims=2),
                                                              std(X, dims=2))
                ...
                @testset "StatsBase.cov2cor!" begin
                    test_isapprox_preserves_symherm_structure(StatsBase.cov2cor!,
                                                              cov(X, dims=1),
                                                              cor(X, dims=1),
                                                              std(X, dims=1))
                    test_isapprox_preserves_symherm_structure(StatsBase.cov2cor!,
                                                              ...
                                                              cor(X, dims=2),
                                                              std(X, dims=2))
                    test_isapprox_preserves_symherm_structure(StatsBase.cov2cor!, cov1,
                                                              cor1)
                    test_isapprox_preserves_symherm_structure(StatsBase.cov2cor!, cov2,
                                                              cor2)
                    test_isapprox_preserves_symherm_structure(StatsBase.cov2cor!, cov1,
                                                              cor1, std1)
                    test_isapprox_preserves_symherm_structure(StatsBase.cov2cor!, cov2,
                ...
                @testset "cor2cov" begin
                    test_isapprox_preserves_symherm_structure(cor2cov, cor(X, dims=1),
                                                              cov(X, dims=1),
                                                              std(X, dims=1))
                    test_isapprox_preserves_symherm_structure(cor2cov, cor(X, dims=2),
                                                              cov(X, dims=2),
                                                              std(X, dims=2))
                ...
                @testset "StatsBase.cor2cov!" begin
                    test_isapprox_preserves_symherm_structure(StatsBase.cor2cov!,
                                                              cor(X, dims=1),
                                                              cov(X, dims=1),
                                                              std(X, dims=1))
                    test_isapprox_preserves_symherm_structure(StatsBase.cor2cov!,
                                                              ...
                                                              cov(X, dims=2),
                                                              std(X, dims=2))
                    test_isapprox_preserves_symherm_structure(StatsBase.cor2cov!, cor1,
                                                              cov1, std1)
                    test_isapprox_preserves_symherm_structure(StatsBase.cor2cov!, cor2,
                                                              cov2, std2)
                end
            end
        end
        ...
        @testset "Corrected" begin
            @testset "Weighted Covariance" begin
                if isa(wv1, Weights)
                    ...
                    @test StatsBase.covm(X, mean(X, wv1, dims=1), wv1, 1; corrected=true) ≈
                          S1w .* var_corr1
                    @test StatsBase.covm(X, mean(X, wv2, dims=2), wv2, 2; corrected=true) ≈
                          S2w .* var_corr2

                    @test StatsBase.covm(X, zeros(1, 8), wv1, 1; corrected=true) ≈
                          Sz1w .* var_corr1
                    @test StatsBase.covm(X, zeros(3), wv2, 2; corrected=true) ≈
                          Sz2w .* var_corr2
                end
            end
            @testset "Mean and covariance" begin
        ...
        @testset "Abstract covariance estimation" begin
            ...
            for corrected in (false, true)
                ...
                @test cov(scc, X, mean=Xm2, dims=2) ≈
                      StatsBase.covm(X, Xm2, 2, corrected=corrected)
                if f !== weights || corrected === false
                    ...
                    @test cov(scc, X, wv1, mean=Xm1) ≈
                          StatsBase.covm(X, Xm1, wv1, corrected=corrected)
                    @test cov(scc, X, wv2, mean=Xm2, dims=2) ≈
                          StatsBase.covm(X, Xm2, wv2, 2, corrected=corrected)
                end
            end
        end
    end
