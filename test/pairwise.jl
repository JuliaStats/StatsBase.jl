...
@testset "pairwise and pairwise! with $f" for f in (arbitrary_fun, cor, cov)
    @testset "basic interface" begin
        ...
        @test_throws DimensionMismatch pairwise!(
            f, zeros(0, 0),
            [], [[1, 2], [2, 3]]
        )
    end
    ...
    @testset "missing values handling interface" begin
        ...
        @test res ≅ res2
        @test isapprox(
            res, [f(collect.(skipmissings(xi, yi))...) for xi in xm, yi in ym],
            rtol = 1.0e-6
        )
        ...
        @test res ≅ res2
        @test isapprox(
            res, [f(collect.(skipmissings(yi, zi))...) for yi in ym, zi in zm],
            rtol = 1.0e-6
        )
        ...
        @test res ≅ res2
        @test isapprox(
            res, [f(view(xi, nminds), view(yi, nminds)) for xi in xm, yi in ym],
            rtol = 1.0e-6
        )
        ...
        @test_throws ArgumentError pairwise!(
            f, zeros(
    ...
    @testset "cor corner cases" begin
        ...
        @test res isa Matrix{Float64}
        @test res == [
            cor(xi, yi) for xi in ([1, 2, 3], [1, 5, 2]),
                yi in ([1, 2, 3], [1, 5, 2])
        ]
