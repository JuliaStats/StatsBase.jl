...
@testset "StatsBase.Histogram" begin
    @testset "Histogram binindex and binvolume" begin
        ...
        @test [StatsBase.binvolume(h2, (i, j))
               for i in axes(h2.weights, 1), j in axes(h2.weights, 2)] ≈
              diff(edg1) * diff(edg2)'
    ...
    @testset "Histogram append" begin
        ...
        @test append!(Histogram(0:20:100, Float64, :left, false), 0:0.5:99.99).weights ≈
              [40, 40, 40, 40, 40]
        @test append!(Histogram(0:20:100, Float64, :left, true), 0:0.5:99.99).weights ≈
              [2, 2, 2, 2, 2]
        @test append!(Histogram(0:20:100, Float64, :left, false), 0:0.5:99.99,
                      fill(2, 200)).weights ≈ [80, 80, 80, 80, 80]
        @test append!(Histogram(0:20:100, Float64, :left, true), 0:0.5:99.99,
                      fill(2, 200)).weights ≈ [4, 4, 4, 4, 4]
    end
    ...
    @testset "Histogram fit" begin
        ...
        @test fit(Histogram, 1:100, nbins=5, closed=:left).weights ==
              [19, 20, 20, 20, 20, 1]
        @test fit(Histogram, 0:99, nbins=5, closed=:right).weights ==
              [1, 20, 20, 20, 20, 19]
        @test fit(Histogram, 0:99, nbins=5, closed=:left).weights == [20, 20, 20, 20, 20]

        @test fit(Histogram, (0:99, 0:99), nbins=5).weights ==
              Matrix(Diagonal([20, 20, 20, 20, 20]))
        @test fit(Histogram, (0:99, 0:99), nbins=(5, 5)).weights ==
              Matrix(Diagonal([20, 20, 20, 20, 20]))

        @test fit(Histogram, 0:99, weights(ones(100)), nbins=5).weights ==
              [20, 20, 20, 20, 20]
        @test fit(Histogram, 0:99, weights(2*ones(100)), nbins=5).weights ==
              [40, 40, 40, 40, 40]
        @test fit(Histogram{Int32}, 0:99, weights(2*ones(100)), nbins=5).weights ==
              [40, 40, 40, 40, 40]
        @test fit(Histogram{Float32}, 0:99, weights(2*ones(100)), nbins=5).weights ==
              [40, 40, 40, 40, 40]

        d = collect(0:99)
        v = view(d, fill(true, 100))
        @test fit(Histogram{Float32}, v, weights(2*ones(100)), nbins=5).weights ==
              [40, 40, 40, 40, 40]
    end

    @testset "Histogram element type" begin
        @test eltype(@inferred(fit(Histogram, 1:100, weights(ones(Int, 100)), nbins=5)).weights) ==
              Int
        @test eltype(@inferred(fit(Histogram{Float32}, 1:100, weights(ones(Int, 100)),
                                   nbins=5)).weights) == Float32
        @test eltype(@inferred(fit(Histogram, 1:100, weights(ones(Float64, 100)), nbins=5)).weights) ==
              Float64
        @test eltype(@inferred(fit(Histogram{Float32}, 1:100, weights(ones(Float64, 100)),
                                   nbins=5)).weights) == Float32
    end
    ...
    @testset "histrange" begin
        ...
        @test_throws ArgumentError StatsBase.histrange([0.0], 0, :left)
    end
    ...
    @testset "Histogram norm and normalize" begin
        ...
        edges = (cumsum(rand(rng) * rand(rng, 9)),
                 cumsum(rand(rng, 1:10) * rand(rng, 1:100, 11)),
                 cumsum(5 * rand(rng) * rand(rng, 14)))

        n = 100000

        data = (maximum(edges[1]) .* (randn(rng, n) ./ 6 .+ 0.5),
        ...
        bin_vols = [x * y * z
                    for x in diff(edges[1]), y in diff(edges[2]), z in diff(edges[3])]
        ...
        @test @inferred(normalize(h_density, mode=:pdf)) ==
              Histogram(h_density.edges, h_density.weights .* (1/norm(h_density)),
                        h_density.closed, true)
        ...
        divor0_cmp = (a_n, a_d, b_n, b_d) -> maximum(abs.(map(divor0, a_n, a_d) -
                                                          map(divor0, b_n, b_d))) < 1e-10
        ...
        @test divor0_cmp(h_density3_aux, h_density2_aux, h_density3.weights,
                         h_density2.weights)
    end
    ...
    @testset "Histogram merge" begin
        histograms = [fit(Histogram, (rand(100), 10 * rand(100)), (0:0.1:1, 0:1:10))
                      for _ in 1:10]
        h = zero(histograms[1])
        merge!(h, histograms ...)
