...
m = [
    0.11258244478647295  0.05664544616214151  0.38181274408522614  0.8197779704008801
...
@test sprint(show, PValue(1.0e-1)) == "0.1000"
@test sprint(show, PValue(1.0e-5)) == "<1e-04"
...
@test sprint(show, TestStat(1.0e-1)) == "0.10"
@test sprint(show, TestStat(1.0e-5)) == "0.00"
...
@testset "StatsAPI model reexports" begin
    for f in (
            fitted, response, responsename, meanresponse,
