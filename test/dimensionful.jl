# This file was formerly part of Julia. License is MIT: https://julialang.org/license
...
for op in (:+, :-, :middle, :hypot)
    @eval function Base.$op(x::Furlong{p}, y::Furlong{p}) where {p}
        v = $op(x.val, y.val)
        Furlong{p}(v)
    end
end
for op in (:(==), :(!=), :<, :<=, :isless, :isequal)
    @eval Base.$op(x::Furlong{p}, y::Furlong{p}) where {p} = $op(x.val, y.val)
end
...
for (op,eop) in ((:*, :_plus), (:/, :_minus), (://, :_minus), (:div, :_minus))
    @eval begin
        Base.$op(x::Furlong{p}, y::Furlong{q}) where {p,q} =
            $eop($op(x.val, y.val),x,y)
        Base.$op(x::Furlong{p}, y::S) where {p,S<:Number} = $op(x,Furlong{0,S}(y))
        Base.$op(x::S, y::Furlong{p}) where {p,S<:Number} = $op(Furlong{0,S}(x),y)
    end
end
for op in (:rem, :mod)
    @eval begin
        Base.$op(x::Furlong{p}, y::Furlong) where {p} = Furlong{p}($op(x.val, y.val))
        Base.$op(x::Furlong{p}, y::Number) where {p} = Furlong{p}($op(x.val, y))
    end
end
