...
@inline function varcorrection(w::Weights, corrected::Bool = false)
    corrected && throw(
        ArgumentError(
            "Weights type does not support bias correction: " *
...
@generated function _wsum_general!(
        R::AbstractArray{RT}, f::supertype(typeof(abs)),
    ...
    return quote
        ...
        if dim == 1
            ...
            ) (@nref $N R j) += f(@nref $N A i) * wi
        end
        return R
    end
end

@generated function _wsum_centralize!(
        R::AbstractArray{RT}, f::supertype(typeof(abs)),
    ...
    return quote
        ...
        if dim == 1
            ...
            ) (@nref $N R j) += f((@nref $N A i) - (@nref $N means j)) * wi
        end
        return R
    end
end
...
function wsum(A::AbstractArray{T}, w::AbstractVector{W}, dim::Int) where {T <: Number, W <: Real}
    length(w) == size(A, dim) || throw(DimensionMismatch("Inconsistent array dimension."))
    return _wsum!(similar(A, wsumtype(T, W), Base.reduced_indices(axes(A), dim)), A, w, dim, true)
end
...
function quantile(v::AbstractVector{V}, w::AbstractWeights{W}, p::AbstractVector{<:Real}) where {V, W <: Real}
    ...
    length(v) == length(w) || throw(
        ArgumentError(
            "data and weight vectors must be the same size," *
    ...
    isa(w, FrequencyWeights) && !(eltype(w) <: Integer) && any(!isinteger, w) &&
        throw(
        ArgumentError(
            "The values of the vector of `FrequencyWeights` must be numerically" *
