...
function check_lags(lx::Int, lags::AbstractVector)
    return (maximum(lags) < lx || error("lags must be less than the sample length."))
end

function demean_col!(z::AbstractVector{<:Real}, x::AbstractMatrix{<:Real}, j::Int,
                     demean::Bool)
    T = eltype(z)
    m = size(x, 1)
    @assert m == length(z)
...
function _autodot(x::AbstractVector{<:Union{Float32,Float64}}, lx::Int, l::Int)
    return dot(x, 1:(lx - l), x, (1 + l):lx)
end
function _autodot(x::AbstractVector{<:Real}, lx::Int, l::Int)
    return dot(view(x, 1:(lx - l)), view(x, (1 + l):lx))
end
...
"""
function autocov!(r::AbstractVector{<:Real}, x::AbstractVector{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    lx = length(x)
    m = length(lags)
    ...
    for k in 1:m  # foreach lag value
        r[k] = _autodot(z, lx, lags[k]) / lx
    end
    return r
end

function autocov!(r::AbstractMatrix{<:Real}, x::AbstractMatrix{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    lx = size(x, 1)
    ns = size(x, 2)
    m = length(lags)
...
"""
function autocov(x::AbstractVector{<:Real}, lags::AbstractVector{<:Integer};
                 demean::Bool=true)
    out = Vector{float(eltype(x))}(undef, length(lags))
    return autocov!(out, x, lags; demean=demean)
end

function autocov(x::AbstractMatrix{<:Real}, lags::AbstractVector{<:Integer};
                 demean::Bool=true)
    out = Matrix{float(eltype(x))}(undef, length(lags), size(x, 2))
    return autocov!(out, x, lags; demean=demean)
end

function autocov(x::AbstractVecOrMat{<:Real}; demean::Bool=true)
    return autocov(x, default_autolags(size(x, 1)); demean=demean)
end
...
"""
function autocor!(r::AbstractVector{<:Real}, x::AbstractVector{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    lx = length(x)
    m = length(lags)
    length(r) == m || throw(DimensionMismatch())
...
function autocor!(r::AbstractMatrix{<:Real}, x::AbstractMatrix{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    lx = size(x, 1)
    ns = size(x, 2)
    m = length(lags)
...
autocorrelation is 1. See [`autocov`](@ref) for the unnormalized form.
"""
function autocor(x::AbstractVector{<:Real}, lags::AbstractVector{<:Integer};
                 demean::Bool=true)
    out = Vector{float(eltype(x))}(undef, length(lags))
    return autocor!(out, x, lags; demean=demean)
end

function autocor(x::AbstractMatrix{<:Real}, lags::AbstractVector{<:Integer};
                 demean::Bool=true)
    out = Matrix{float(eltype(x))}(undef, length(lags), size(x, 2))
    return autocor!(out, x, lags; demean=demean)
end

function autocor(x::AbstractVecOrMat{<:Real}; demean::Bool=true)
    return autocor(x, default_autolags(size(x, 1)); demean=demean)
end
...
function _crossdot(x::AbstractVector{T}, y::AbstractVector{T}, lx::Int,
                   l::Int) where {T<:Union{Float32,Float64}}
    if l >= 0
...
"""
function crosscov!(r::AbstractVector{<:Real}, x::AbstractVector{<:Real},
                   y::AbstractVector{<:Real}, lags::AbstractVector{<:Integer};
                   demean::Bool=true)
    lx = length(x)
    m = length(lags)
...
function crosscov!(r::AbstractMatrix{<:Real}, x::AbstractMatrix{<:Real},
                   y::AbstractVector{<:Real}, lags::AbstractVector{<:Integer};
                   demean::Bool=true)
    lx = size(x, 1)
    ns = size(x, 2)
...
function crosscov!(r::AbstractMatrix{<:Real}, x::AbstractVector{<:Real},
                   y::AbstractMatrix{<:Real}, lags::AbstractVector{<:Integer};
                   demean::Bool=true)
    lx = length(x)
    ns = size(y, 2)
    m = length(lags)
...
function crosscov!(r::AbstractArray{<:Real,3}, x::AbstractMatrix{<:Real},
                   y::AbstractMatrix{<:Real}, lags::AbstractVector{<:Integer};
                   demean::Bool=true)
    lx = size(x, 1)
    nx = size(x, 2)
...
The output is not normalized. See [`crosscor`](@ref) for a function with normalization.
"""
function crosscov(x::AbstractVector{<:Real}, y::AbstractVector{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    out = Vector{float(Base.promote_eltype(x, y))}(undef, length(lags))
    return crosscov!(out, x, y, lags; demean=demean)
end

function crosscov(x::AbstractMatrix{<:Real}, y::AbstractVector{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    out = Matrix{float(Base.promote_eltype(x, y))}(undef, length(lags), size(x, 2))
    return crosscov!(out, x, y, lags; demean=demean)
end

function crosscov(x::AbstractVector{<:Real}, y::AbstractMatrix{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    out = Matrix{float(Base.promote_eltype(x, y))}(undef, length(lags), size(y, 2))
    return crosscov!(out, x, y, lags; demean=demean)
end

function crosscov(x::AbstractMatrix{<:Real}, y::AbstractMatrix{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    out = Array{float(Base.promote_eltype(x, y)),3}(undef, length(lags), size(x, 2),
                                                    size(y, 2))
    return crosscov!(out, x, y, lags; demean=demean)
end

function crosscov(x::AbstractVecOrMat{<:Real}, y::AbstractVecOrMat{<:Real};
                  demean::Bool=true)
    return crosscov(x, y, default_crosslags(size(x, 1)); demean=demean)
end
...
unnormalized form.
"""
function crosscor!(r::AbstractVector{<:Real}, x::AbstractVector{<:Real},
                   y::AbstractVector{<:Real}, lags::AbstractVector{<:Integer};
                   demean::Bool=true)
    lx = length(x)
    m = length(lags)
...
function crosscor!(r::AbstractMatrix{<:Real}, x::AbstractMatrix{<:Real},
                   y::AbstractVector{<:Real}, lags::AbstractVector{<:Integer};
                   demean::Bool=true)
    lx = size(x, 1)
    ns = size(x, 2)
...
function crosscor!(r::AbstractMatrix{<:Real}, x::AbstractVector{<:Real},
                   y::AbstractMatrix{<:Real}, lags::AbstractVector{<:Integer};
                   demean::Bool=true)
    lx = length(x)
    ns = size(y, 2)
...
function crosscor!(r::AbstractArray{<:Real,3}, x::AbstractMatrix{<:Real},
                   y::AbstractMatrix{<:Real}, lags::AbstractVector{<:Integer};
                   demean::Bool=true)
    lx = size(x, 1)
    nx = size(x, 2)
...
unnormalized form.
"""
function crosscor(x::AbstractVector{<:Real}, y::AbstractVector{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    out = Vector{float(Base.promote_eltype(x, y))}(undef, length(lags))
    return crosscor!(out, x, y, lags; demean=demean)
end

function crosscor(x::AbstractMatrix{<:Real}, y::AbstractVector{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    out = Matrix{float(Base.promote_eltype(x, y))}(undef, length(lags), size(x, 2))
    return crosscor!(out, x, y, lags; demean=demean)
end

function crosscor(x::AbstractVector{<:Real}, y::AbstractMatrix{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    out = Matrix{float(Base.promote_eltype(x, y))}(undef, length(lags), size(y, 2))
    return crosscor!(out, x, y, lags; demean=demean)
end

function crosscor(x::AbstractMatrix{<:Real}, y::AbstractMatrix{<:Real},
                  lags::AbstractVector{<:Integer}; demean::Bool=true)
    out = Array{float(Base.promote_eltype(x, y)),3}(undef, length(lags), size(x, 2),
                                                    size(y, 2))
    return crosscor!(out, x, y, lags; demean=demean)
...
function crosscor(x::AbstractVecOrMat{<:Real}, y::AbstractVecOrMat{<:Real};
                  demean::Bool=true)
    return crosscor(x, y, default_crosslags(size(x, 1)); demean=demean)
end
...
function pacf_regress!(r::AbstractMatrix{<:Real}, X::AbstractMatrix{<:Real},
                       lags::AbstractVector{<:Integer}, mk::Integer)
    lx = size(X, 1)
    ...
    for j in 1:size(X, 2)
        ...
        for i in 1:length(lags)
            ...
            r[i, j] = l == 0 ? 1 :
                      (cholesky!(sX'sX, Val(false)) \ (sX'view(X, (1 + l):lx, j)))[end]
        end
    end
    return r
end

function pacf_yulewalker!(r::AbstractMatrix{<:Real}, X::AbstractMatrix{T},
                          lags::AbstractVector{<:Integer},
                          mk::Integer) where {T<:Union{Float32,Float64}}
...
"""
function pacf!(r::AbstractMatrix{<:Real}, X::AbstractMatrix{T},
               lags::AbstractVector{<:Integer};
               method::Symbol=:regression) where {T<:Union{Float32,Float64}}
    lx = size(X, 1)
    m = length(lags)
...
function pacf(X::AbstractMatrix{<:Real}, lags::AbstractVector{<:Integer};
    ...
    out = Matrix{float(eltype(X))}(undef, length(lags), size(X, 2))
    return pacf!(out, float(X), lags; method=method)
end

function pacf(x::AbstractVector{<:Real}, lags::AbstractVector{<:Integer};
              method::Symbol=:regression)
    return vec(pacf(reshape(x, length(x), 1), lags; method=method))
end
