...
function Base.isapprox(x1::Union{TestStat,PValue}, x2::Union{TestStat,PValue}; kwargs...)
    return isapprox(x1.v, x2.v; kwargs...)
end
...
mutable struct CoefTable
    ...
    function CoefTable(cols::Vector, colnms::Vector, rownms::Vector,
        ...
        nrs = map(length, cols)
        nr = nrs[1]
        length(colnms) in [0, nc] ||
            throw(ArgumentError("colnms should have length 0 or $nc"))
        length(rownms) in [0, nr] ||
            throw(ArgumentError("rownms should have length 0 or $nr"))
        all(nrs .== nr) ||
            throw(ArgumentError("Elements of cols should have equal lengths, but got $nrs"))
        pvalcol in 0:nc || throw(ArgumentError("pvalcol should be between 0 and $nc"))
        teststatcol in 0:nc ||
            throw(ArgumentError("teststatcol should be between 0 and $nc"))
...
struct ConvergenceException{T<:Real} <: Exception
    ...
    tol::T
    msg::String
    function ConvergenceException{T}(iters, lastchange::T, tol::T,
...
function ConvergenceException(iters, lastchange::T=NaN, tol::T=NaN,
                              msg::AbstractString="") where {T<:Real}
    return ConvergenceException{T}(iters, lastchange, tol, String(msg))
end
