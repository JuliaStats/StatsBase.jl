...
function inverse_rle(vals::AbstractVector{T}, lens::AbstractVector{<:Integer}) where {T}
    ...
    mlens = length(lens)
    mlens == m ||
        throw(DimensionMismatch("number of vals ($m) does not match the number of lens ($mlens)"))
    n = sum(lens)
...
function indicatormat(x::AbstractArray; sparse::Bool=false)
    return indicatormat(x, sort!(unique(x)); sparse=sparse)
end
...
function _indicatormat_sparse(x::AbstractArray{<:Integer}, k::Integer)
    return (n=length(x); sparse(x, 1:n, true, k, n))
end
