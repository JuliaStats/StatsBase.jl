...
"""
Base.varm(v::RealArray, wv::AbstractWeights, m::Real) = _moment2(v, wv, m)
...
Base.varm!(R::AbstractArray, A::RealArray, wv::AbstractWeights, M::RealArray, dim::Int) =
    scale!(_wsum_centralize!(R, @functorize(abs2), A, values(wv), M, dim, true), inv(bias(wv)))
...
Base.varm(A::RealArray, wv::AbstractWeights, M::RealArray, dim::Int) =
...
Base.var(A::RealArray, wv::AbstractWeights, dim::Int; mean=nothing) =
    @static if VERSION < v"0.6.0-dev.1121"
...
"""
Base.stdm(v::RealArray, wv::AbstractWeights, m::Real) = sqrt(varm(v, wv, m))
...
"""
Base.std(v::RealArray, wv::AbstractWeights; mean=nothing) = sqrt.(var(v, wv; mean=mean))

Base.stdm(v::RealArray, m::RealArray, dim::Int) = Base.sqrt!(varm(v, m, dim))
Base.stdm(v::RealArray, wv::AbstractWeights, m::RealArray, dim::Int) = sqrt.(varm(v, wv, m, dim))
Base.std(v::RealArray, wv::AbstractWeights, dim::Int; mean=nothing) = sqrt.(var(v, wv, dim; mean=mean))
...
mean_and_var(A::RealArray, wv::AbstractWeights) = (m = mean(A, wv); (m, varm(A, wv, m)))
mean_and_std(A::RealArray, wv::AbstractWeights) = (m = mean(A, wv); (m, stdm(A, wv, m)))

mean_and_var(A::RealArray, dim::Int) = (m = mean(A, dim); (m, varm(A, m, dim)))
mean_and_std(A::RealArray, dim::Int) = (m = mean(A, dim); (m, stdm(A, m, dim)))

mean_and_var(A::RealArray, wv::AbstractWeights, dim::Int) = (m = mean(A, wv, dim); (m, varm(A, wv, m, dim)))
mean_and_std(A::RealArray, wv::AbstractWeights, dim::Int) = (m = mean(A, wv, dim); (m, stdm(A, wv, m, dim)))
...
moment(v::RealArray, k::Int, wv::AbstractWeights) = moment(v, k, wv, mean(v, wv))
